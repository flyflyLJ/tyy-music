...
import com.tyy.ss.search.entity.netease.NeteaseMusicBase;
import com.tyy.ss.search.entity.netease.NeteaseMusicLyr;
import com.tyy.ss.search.entity.netease.NeteaseMusicPic;
import com.tyy.ss.search.entity.netease.NeteaseMusicUrl;
import com.tyy.ss.search.entity.vo.SearchRequestVo;
import com.tyy.ss.search.musicEnum.MusicSourceEnum;
import com.tyy.ss.search.service.NeteaseMusicService;
import com.tyy.ss.search.util.CommonUtils;
...
public class NeteaseMusicServiceImpl implements NeteaseMusicService {
    ...
    private static String domain= "https://music.163.com/#";
    ...
    public List<Music> findMusicByPage(SearchRequestVo req) {
        ...
        String url = String.format("%s/search/m?s=%s&limit=%s&offset=%s", domain, req.getName(), limit, offset);
        NeteaseMusicBase neteaseMusicBase = OkHttpUtils.getRequest(url, null, null, NeteaseMusicBase.class);
        List<NeteaseMusicBase.ResultBean.SongsBean> songs = neteaseMusicBase.getResult().getSongs();
        for (NeteaseMusicBase.ResultBean.SongsBean song : songs) {
            Music music = new Music();
            music.setName(song.getName());
            String artist = "";
            List<NeteaseMusicBase.ResultBean.SongsBean.ArtistsBean> artists = song.getArtists();
